    ...
    .array(
      z.object({
        ...
        item: z
          .string()
          .describe(
            "品目。あとで示す『使える品目』から選ぶのが基本（表記が割れると集計できなくなるため）。ただし既存にぴったり合うものが無く、今後も繰り返し買うものなら、新しい品目名を書いてよい。経費・資産・負債の行は空。",
          ),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (items.length) {
      system += `\n# 使える品目（基本はこの中から選ぶ）\n[${items.join(", ")}]\n` +
        "※ぴったり合うものが無く、今後も繰り返し買うものなら新しい品目名を提案してよい。" +
        "その場合は reply で「品目『○○』を新しく作ります」と一言添える。" +
        "ユーザーから品目名の指定があれば、リストに無くてもその名前をそのまま使う。";
    }
  } catch {
    /* KV未設定でも継続 */
  }
